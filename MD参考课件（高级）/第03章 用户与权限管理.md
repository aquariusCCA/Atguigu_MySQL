...
# 7. 系統變量
## 7.1 🔧 MySQL 系統變量簡介
MySQL 的系統變量（System Variables）是影響資料庫運行行為的重要參數，分為兩大類：

| 類型 | 說明 |
|------|------|
| **全域變量（Global）** | 作用於整個伺服器，影響所有連線與會話 |
| **會話變量（Session）** | 只作用於當前連線會話，不影響其他使用者 |

這些變量可以在：
- **配置文件**（如 `my.cnf` / `my.ini`）中設定（伺服器啟動時讀取）
- **執行期間** 使用 `SET` 指令動態修改（部分需有 SUPER 權限）
...
| 系統變量 | 說明 |
|----------|------|
| `max_connections` | 允許的最大連線數，預設為 151 |
| `sql_mode` | 控制 SQL 語法行為，如是否允許隱式轉換 |
| `innodb_buffer_pool_size` | InnoDB 的緩衝區大小，影響記憶體使用效率 |
| `default_storage_engine` | 預設儲存引擎（如 InnoDB） |
| `wait_timeout` | 等待連線逾時時間，預設 28800 秒 |
| `log_bin` | 是否開啟二進位日誌 |
| `character_set_server` | 預設的伺服器編碼 |
...
```sql
-- 查詢 max_connections 的全域值（GLOBAL）
SELECT @@GLOBAL.max_connections;

-- 查詢 wait_timeout 的當前會話值（SESSION）
...
#### （2）**全域級修改（影響所有連線）**
> 需有 `SUPER` 或 `SYSTEM_VARIABLES_ADMIN` 權限
```sql
SET GLOBAL max_connections = 200;
```

> ⚠️ 注意：有些變量改了後不會永久保存，**重啟 MySQL 就失效了**，如果想永久生效，需寫入 `my.cnf` 配置檔案中。
...
| 比較項 | 系統變量（System Variables） | 狀態變量（Status Variables） |
|--------|------------------------------|------------------------------|
| 功能 | 控制 MySQL 行為 | 反映 MySQL 執行狀態 |
| 查詢方式 | `SHOW VARIABLES` | `SHOW STATUS` |
| 範例 | `max_connections`、`sql_mode` | `Connections`、`Threads_connected` |

---

如果你有任何特定的變量想深入了解（例如 `innodb_buffer_pool_size` 或 `sql_mode` 的模式設定），我也可以進一步幫你解析！需要嗎？

## 7.2 🔍 查看 MySQL 系統變量

MySQL 提供了多種方式來查看系統變量的 **名稱、值與範圍**，這些方式可幫助你掌握目前資料庫的行為設定。
...
```sql
-- 查看所有全局變量（GLOBAL），適用於所有連線
SHOW GLOBAL VARIABLES;

-- 查看當前會話的變量（SESSION），只針對本連線有效
SHOW SESSION VARIABLES;

-- 這個也可以用於查看當前連線（其實預設就是 SESSION）
SHOW VARIABLES;
```
...
由於變量非常多，建議搭配 `LIKE` 模糊查詢關鍵字：

```sql
-- 範例：查詢所有名稱中包含 "timeout" 的變量
...
> 說明：這表示目前 MySQL 伺服器最多允許 151 個客戶端同時連線。
...
> 常見的變量包括：
- `character_set_server`：伺服器預設編碼（例如 `utf8mb4`）
- `character_set_client`：用戶端資料傳送的編碼
- `character_set_connection`：SQL 字串與資料庫的轉換編碼

---

#### 範例 3：查詢 InnoDB 的相關變量
...
- `innodb_log_file_size`：日誌檔大小
- `innodb_flush_log_at_trx_commit`：控制事務提交時日誌寫入策略
...
| 通配符 | 說明 | 範例 |
|--------|------|------|
| `%`    | 任意多個字元 | `'%timeout%'` 可匹配 `wait_timeout`, `net_read_timeout` 等 |
| `_`    | 任意單一字元 | `'innodb_buffer_pool_s%z%'` |
...
```sql
-- 查詢當前 session 的 wait_timeout 值
SELECT @@SESSION.wait_timeout;

-- 查詢全域的 innodb_buffer_pool_size 值
SELECT @@GLOBAL.innodb_buffer_pool_size;
```

---

如果你有想查的特定系統變量，也可以告訴我，我可以幫你解釋該變量的用途與如何調整！

## 7.3 设置系统变量
### 7.3.1 通过启动选项设置
#### 🚀 一、透過啟動選項設定系統變量（兩種方式）

MySQL 的系統變量大多可以在「**啟動 MySQL 伺服器時**」進行設定，主要有兩種方式：

---

##### ✅ 方式一：透過命令列設定啟動選項

啟動 `mysqld` 伺服器時，在命令中指定系統變量：
...
##### ✅ 方式二：透過 MySQL 配置檔案（`my.cnf` 或 `my.ini`）

編輯 MySQL 的設定檔，位於 `/etc/my.cnf`（Linux）或 `C:\ProgramData\MySQL\MySQL Server X.Y\my.ini`（Windows）中。
...
> ✅ 注意事項：
- `[mysqld]` 或 `[server]` 段指定的是 MySQL Server 本身的設定
- 配置檔案可在伺服器啟動時自動載入設定
- **重啟 MySQL 伺服器** 後設定才會生效！

---

#### 🔄 二、單詞連接規則小技巧

| 啟動選項（命令列/配置檔）     | 系統變量名稱（SHOW VARIABLES 查詢） |
|-------------------------------|-------------------------------------|
| `--max-connections=100`       | `max_connections`                   |
| `--default-storage-engine`    | `default_storage_engine`            |
| `--innodb-buffer-pool-size`   | `innodb_buffer_pool_size`           |

✅ **說明重點：**
- **命令列 / 配置檔案：** 可以使用「短橫線（`-`）」或「底線（`_`）」
- **SQL 層級系統變量名稱（SHOW VARIABLES）：** 必須使用「底線（`_`）」

---

#### 🧪 三、範例操作與驗證

##### ✅ 使用命令列啟動 MySQL 並設置變量（模擬）

```bash
mysqld --default-storage-engine=MyISAM --max-connections=50
```

接著進入 MySQL 來驗證設定是否生效：
...
1. **變數是否支援啟動設置？**
   - 並非所有變數都能在啟動時設置。可參考官方文檔或使用 `SHOW VARIABLES` 查驗。
2. **變更系統變量後沒效果？**
   - 檢查是否有重啟 MySQL，因為啟動選項必須在伺服器啟動時套用。
3. **動態變更 vs 啟動選項設定：**
   - 動態變更使用 `SET GLOBAL xxx`，但 **不會永久保存**
   - 若希望永久生效，需透過 **啟動選項或設定檔** 設定

---

如果你需要我協助設計 `my.cnf` 設定模板，或希望了解某個變量能否設置為啟動選項，也可以再告訴我！

### 7.3.2 服务器程序运行过程中设置
#### 7.3.2.1 設置不同作用範圍的系統變量
##### ✅ 一、MySQL 系統變量的**作用範圍**

| 作用範圍 | 說明 | 影響範圍 |
|----------|------|-----------|
| `GLOBAL` | 全域變量 | 對所有用戶有效（包括未來新連線） |
| `SESSION` / `LOCAL` | 會話變量 | 僅對當前連線有效 |

---

##### 🔄 二、變量初始化邏輯

- MySQL 伺服器啟動時，會根據預設或啟動參數，設定**GLOBAL 變量**。
- 當有用戶端連線進來時，MySQL 會：
  - **複製 GLOBAL 變量的值**初始化為該用戶端的 SESSION 變量。
  - 之後用戶端可以自行調整 SESSION 變量，不會影響其他人。
...
> 🔁 生效對象：從設定之後**新連線進來的用戶端**，會使用此值初始化 SESSION 變量。
...
> ✅ 結論：**兩個客戶端各自獨立設定，不互相影響**

---

###### 🔄 如果你要讓「所有新連線」都預設用 MyISAM
...
-- 客戶端 C 新連進來後執行
SELECT @@default_storage_engine;
-- => MyISAM （因為此 SESSION 初始化於 GLOBAL）
```
...
1. ✅ **省略作用範圍，預設為 SESSION**
   ```sql
   SET sql_mode = 'STRICT_ALL_TABLES';  -- 等同 SET SESSION sql_mode
   ```

2. 🔒 設定 GLOBAL 變量需要具有 `SUPER` 或 `SYSTEM_VARIABLES_ADMIN` 權限。

3. ❌ 有些變量是 **read-only**（只讀）或無法動態設定，必須透過配置檔並重啟 MySQL 才能生效。

---

##### 📚 六、常見變量適用範圍整理

| 變量名稱                  | 預設作用範圍 | 可否動態設定 | 說明 |
|---------------------------|---------------|----------------|------|
| `max_connections`         | GLOBAL        | ✅              | 最大同時連線數 |
| `wait_timeout`            | SESSION/GLOBAL| ✅              | 等待逾時秒數 |
| `sql_mode`                | SESSION/GLOBAL| ✅              | SQL 模式 |
| `default_storage_engine`  | SESSION/GLOBAL| ✅              | 預設儲存引擎 |
| `innodb_buffer_pool_size` | GLOBAL        | ❌（只限配置） | InnoDB 記憶體池大小 |

---

如果你還想練習幾個不同變量的操作範例，或者想知道某個變量是否可動態修改、適用於 GLOBAL 還是 SESSION，我也可以幫你查給你！需要的話跟我說 👍

#### 7.3.2.2 查看不同作用范围的系统变量
##### ✅ 一、`SHOW VARIABLES` 查詢的是哪種作用範圍？
...
- 若想明確查詢全域變量（GLOBAL）或會話變量（SESSION），可指定範圍：
  ```sql
  SHOW GLOBAL VARIABLES;
  SHOW SESSION VARIABLES;
  ```
...
##### 🧪 二、範例：完整演示 GLOBAL 與 SESSION 查詢與修改過程

###### ✅ 第一步：查看 `default_storage_engine` 變量（預設值）

```sql
-- 查看 GLOBAL 值
SHOW GLOBAL VARIABLES LIKE 'default_storage_engine';

-- 查看 SESSION 值（或 SHOW VARIABLES）
...
| Variable_name         | Value    |
|-----------------------|----------|
| default_storage_engine | InnoDB  |  ⬅️ 無論 GLOBAL 或 SESSION 都是 InnoDB

---

###### ✅ 第二步：變更 SESSION 的 default_storage_engine

```sql
-- 改變當前用戶端的 SESSION 值為 MyISAM
...
> ✅ 可以明顯看出：**GLOBAL 沒有改變，只有當前 SESSION 改了！**
...
```sql
-- 這個操作需有 SUPER 或 SYSTEM_VARIABLES_ADMIN 權限
SET GLOBAL default_storage_engine = 'MyISAM';
```
...
> ✅ 新連線的 SESSION 值會從 **GLOBAL 當前值**初始化而來。
...
| 行為 | 影響對象 | 備註 |
|------|-----------|------|
| `SET GLOBAL xxx` | 修改 GLOBAL 值 | 只影響 **之後新連線** 的 SESSION 初始化 |
| `SET SESSION xxx` 或 `SET xxx` | 只影響目前這條連線 | 不會影響其他用戶端 |
| `SHOW VARIABLES` | 預設查看 SESSION 值 | 可用 `SHOW GLOBAL VARIABLES` 指定作用域 |
| GLOBAL 改變後 | 其他已連線的 SESSION 不受影響 | 已存在的連線要自行調整 |
...
如果你還想練習多個變量的設定，例如 `sql_mode`、`wait_timeout` 或 `innodb_log_file_size` 等，我也可以提供多組練習題和說明喔！要幫你設計幾題嗎？