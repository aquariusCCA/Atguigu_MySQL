...
在 MySQL 的 InnoDB 存儲引擎中，**缓冲池**是一塊在記憶體中專門用來「暫存磁碟資料頁（Pages）」的區域，這些頁面可能是：
- 表資料頁（Data Page）
- 索引頁（Index Page）
...
👉 換句話說，**你對資料的每一次讀寫，大機率其實是在對緩衝池的內容操作，而不是直接動磁碟**。
...
✅ 這是**乾淨頁（clean page）**，沒有改變內容，不需寫磁碟。
...
InnoDB 的 Buffer Pool 是用來緩存「資料頁、索引頁等」的內存區塊。
