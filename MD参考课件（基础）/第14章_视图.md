...
### 示例 3：使用 `ALGORITHM`
如果我們想要強制 MySQL 使用 `MERGE` 方法來提高性能：
```sql
CREATE OR REPLACE ALGORITHM = MERGE VIEW high_salary_employees AS
SELECT id, name, salary FROM employees WHERE salary > 70000;
```
這樣當我們查詢 `high_salary_employees`，MySQL 會直接將這個查詢合併到主查詢中，而不會創建臨時表，提高效率。

如果我們改成：
```sql
CREATE OR REPLACE ALGORITHM = TEMPTABLE VIEW high_salary_employees AS
SELECT id, name, salary FROM employees WHERE salary > 70000;
```
則 MySQL 會先創建臨時表存儲結果，然後再查詢。
