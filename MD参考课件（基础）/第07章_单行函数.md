...
在 MySQL 中，函數可以分為**單行函數（Single-Row Functions）**和**多行函數（Multiple-Row Functions）**（也稱為**聚合函數**）。

#### **1. 單行函數（Single-Row Functions）**
**單行函數**對每一個輸入行執行操作，並為每一行返回一個結果。它不會影響其他行的數據。
...
- **字串函數（String Functions）**：
  ...
  - `SUBSTRING(str, start, length)`：提取部分字串

- **數學函數（Mathematical Functions）**：
  - `ABS(x)`：返回絕對值
  - `CEIL(x)` / `FLOOR(x)`：向上/向下取整
  - `ROUND(x, d)`：四捨五入到 d 位小數

- **日期和時間函數（Date & Time Functions）**：
  - `NOW()`：返回當前日期和時間
  - `DATE_FORMAT(date, format)`：格式化日期
  - `DATEDIFF(date1, date2)`：計算兩個日期的差異（天數）

- **轉換函數（Conversion Functions）**：
  - `CAST(value AS type)`：轉換數據類型
  - `CONVERT(value, type)`：轉換數據類型

##### **示例**
```sql
SELECT UPPER('mysql'); -- 結果：'MYSQL'
SELECT ROUND(3.14159, 2); -- 結果：3.14
SELECT NOW(); -- 例如：'2025-03-04 12:30:00'
```

---

#### **2. 多行函數（Multiple-Row Functions / 聚合函數）**
**多行函數**（又稱**聚合函數**）對多行數據執行計算，並返回**單個結果**。

##### **常見的多行函數（聚合函數）**
- **計數（Counting）**：
  - `COUNT(*)`：計算行數
  - `COUNT(column_name)`：計算指定列中非 NULL 的行數

- **總和與平均（Sum & Average）**：
  - `SUM(column_name)`：計算總和
...
##### **示例**
```sql
SELECT COUNT(*) FROM employees; -- 返回表中的總行數
SELECT AVG(salary) FROM employees; -- 計算員工的平均薪資
SELECT MAX(age) FROM customers; -- 查詢客戶的最大年齡
SELECT SUM(sales) FROM orders WHERE order_date >= '2025-01-01'; -- 2025 年後的銷售總額
```

##### **GROUP BY 搭配聚合函數**
多行函數通常搭配 `GROUP BY` 使用，以對不同組別的數據進行計算：
```sql
SELECT department, AVG(salary) 
FROM employees 
GROUP BY department; -- 按部門計算平均薪資
```
...
| 函數類型 | 說明 | 示例 |
|----------|------|------|
| 單行函數 | 針對每一行輸入，返回一個結果 | `UPPER(name)`, `ROUND(price, 2)`, `NOW()` |
| 多行函數（聚合函數） | 針對多行輸入，返回一個結果 | `COUNT(*)`, `SUM(salary)`, `AVG(age)` |

> 你可以使用**單行函數**來處理單獨的數據行，例如格式化字串、數字計算等，而**多行函數**則適合用於彙總分析，如統計總數、計算總和或平均值。
...
FROM
	new_user
WHERE
	TO_DAYS(NOW()) - TO_DAYS(regist_time) <= 7
```
...
SELECT
	...
	salary,
	CASE
		job_id WHEN 'IT_PROG' THEN 1.10 * salary
