...
#### **(1) 为什么查询常数？**
SQL 允许在 `SELECT` 语句中查询常数，即在查询结果中增加一列固定的常数列。这列的取值是**我们指定的，而不是从数据表中动态获取的**。

通常，查询数据时不需要额外增加一个常数列，但在某些情况下，比如**整合不同的数据源**，可以使用常数列作为标识。

#### **(2) 查询常数的应用示例**
假设我们有一个 `employees` 表，我们希望查询所有员工的 `last_name`，并为每条记录添加一个固定的 `corporation` 字段（例如 `"尚硅谷"`）。可以这样写：
```sql
...
这样，每一行都增加了一个固定值 `"尚硅谷"` 作为 `corporation`，这在数据合并、数据标记或导出时非常有用。

#### **(3) 结合 UNION 进行数据整合**
如果我们有两个不同来源的数据集，可以用常数列进行标记，并通过 `UNION` 进行整合。例如：
```sql
